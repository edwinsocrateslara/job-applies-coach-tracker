...
export default function ProgressCPage() {
  const [addServiceOpen, setAddServiceOpen] = useState(false);
  const [addOutcomeOpen, setAddOutcomeOpen] = useState(false);
  const [noteText, setNoteText] = useState("");

  // Drawer state
  const [drawerLane, setDrawerLane] = useState<typeof JOB_LANES[0] | null>(null);
  const drawerOpen = drawerLane !== null;

  function openDrawer(lane: typeof JOB_LANES[0]) {
    setDrawerLane(lane);
  }
    ...
    <div className="flex flex-col flex-1" style={{ background: "var(--color-background)" }}>
      ...
      <div style={{ padding: "24px 32px" }}>
        ...
        <div style={{ display: "flex", gap: "24px", alignItems: "flex-start" }}>
          ...
          <div style={{ flex: "0 0 792px", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {/* ── Job Tracker — Concept C ─────────────────────────── */}
            <div style={CARD_STYLE}>
              <h3 style={{ ...SECTION_HEADING, marginBottom: "16px" }}>Job Tracker</h3>
              <div style={{ display: "flex", gap: "12px", alignItems: "flex-start" }}>
                {JOB_LANES.map((lane) => {
                  const color = LANE_COLORS[lane.label];
                  const visibleJobs = lane.jobs.slice(0, MAX_VISIBLE);
                  const overflowCount = Math.max(0, lane.jobs.length - MAX_VISIBLE);
      ...
      <div
        style={{
          ...
          display: "flex",
          flexDirection: "column",
          boxShadow: "-4px 0 24px rgba(0,0,34,0.12)",
      ...
      >
        ...
        >
          <p style={{ fontSize: "15px", fontWeight: 600, color: "#000022", margin: 0, paddingLeft: "6px" }}>
            {drawerLane ? `${drawerLane.label}: ${drawerLane.jobs.length}` : ""}
          </p>
          <button
            onClick={closeDrawer}
            style={{
        ...
        {/* Drawer body — scrollable card list */}
        <div
          style={{
            flex: 1,
            overflowY: "auto",
